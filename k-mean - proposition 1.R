...
### Pierwsza propozycja na konferencje
...
while (sum(mout[,-c(1, ncol(mout))] - XDist) != 0) {
  ...
  qa <- 0.5 # zmiennan która okresla jaki procent najbliższych wartości będzie brany pod uwagę przy liczeniu nowej wartości punktów
  ...
  for (i in 1:nrow(kpoint)){
    for (c in 1:ncol(standdata)){
      ...
      quantile <- quantile(standdata[filter, c], probs = qa)
      filter2 <- which(standdata[filter, c] < quantile) 
      # modyfikacji ulega tylko 2 punkt procedury, dopisałem do niego dwie linijki, dzięki którym określamy jaki procent (qa) najbliższych odległości jest brany pod uwagę obliczania nowych punktów
      kpoint2[i,c] <- mean(standdata[filter2, c]) # w tym miejscu nie jestem pewny co w sytuacji, gdy np. jednej z k-średnich nie będzie w początkowym etapie 
    }
  } 
  # filtrując daną grupę, liczę średnią arytmetyczną dla każdych współrzędnych spełniający warunek pierwszej propozycji na konferencję
