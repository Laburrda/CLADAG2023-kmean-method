...
### Druga propozycja na konferencje
...
grstanddata <- matrix(0, ncol = ncol(data), nrow = nrow(data))
...
while (sum(mout[,-c(1, ncol(mout))] - XDist) != 0) {
  ...
  singlegroup <- c()
  # logiczny wektor singlegroup będzie potrzebny do zapamiętania czy nasz zbior jest jednoelementowy
  ...
  # pętla, za pomocą której sprawdzam, czy któraś z k-średnich jest zbiorem jednoelementowym, jeśli tak to wektor przyjmuje TRUE dla indeksu odpowiadającemu indeksowi grupy
  
  for (r in 1:nrow(data)){
    for (c in 1:ncol(data)){
      gr_index <- indexes[r]
      if (singlegroup[gr_index] == FALSE){
        grstanddata[r, c] <- (data[r, c] - mean(data[setdiff(which(indexes == gr_index), r), c]))/sd(data[setdiff(which(indexes == gr_index), r), c])
      } else {
        grstanddata[r, c] <- standdata[r, c]
      }
    }
  }
  # dokonuje standaryzacji dla każdego z elementów macierzy data wg schematu: 
  # - (element - średnia dla grupy do której należy z pominięciem jego wartości)/odchylenie standardowe też danej grupy z pominięciem wartości tego elementu
  # - dodatkowy warunek, jeśli zbiór danej średniej jest jednoelementowy to przyjmuje wartość, która występuje przy klasycznej standaryzacji
  # za pomocą funkcji setdiff dokonuje różnicy na zbiorach which(indexes == gr_index (- zbiór zawierający indeksy wierszów należacych do danej grupy) i r (indeks rozpatrywanego wiersza) - dzięki temu pomijana jest wartość o indeksie r

  for (i in 1:nrow(kpoint)){
    for (c in 1:ncol(grstanddata)){
      filter <- which(indexes == i)
      kpoint2[i,c] <- mean(grstanddata[filter, c]) # w tym miejscu nie jestem pewny co w sytuacji, gdy np. jednej z k-średnich nie będzie w początkowym etapie 
    }
  } 
  ...
  for (r in 1:nrow(grstanddata)){
    for (i in 1:nrow(kpoint2)){
      dist2[i] <- sqrt(sum((grstanddata[r,] - kpoint2[i, -ncol(kpoint2)]) ** 2))
